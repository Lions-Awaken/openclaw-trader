...
MANIFEST: list[ManifestEntry] = [
    ...
    ManifestEntry(
        name="shadow_opener_morning",
        script="scripts/shadow_position_opener.py",
        pipeline_name="shadow_position_opener",
        schedule="15 7 * * 1-5",
        schedule_desc="7:15 AM PDT weekdays (after morning scanner)",
        expected_steps=[
            "pipeline:fetch_chains",
            "pipeline:open_positions",
        ],
        criticality="medium",
        dependencies=["scanner_morning"],
        output_validator=None,
        freshness_hours=26,
        estimated_claude_cost=0.0,
    ),
    ManifestEntry(
        name="shadow_opener_midday",
        script="scripts/shadow_position_opener.py",
        pipeline_name="shadow_position_opener",
        schedule="30 10 * * 1-5",
        schedule_desc="10:30 AM PDT weekdays (after midday scanner)",
        expected_steps=[
            "pipeline:fetch_chains",
            "pipeline:open_positions",
        ],
    ...
    ManifestEntry(
        name="shadow_performance_rollup",
        script="scripts/shadow_performance_rollup.py",
        pipeline_name="shadow_performance_rollup",
        schedule="0 9 * * 0",
        schedule_desc="9:00 AM PDT Sundays",
        expected_steps=[
            "shadow_rollup:fetch_profiles",
            "shadow_rollup:fetch_live_pnl",
            "shadow_rollup:notify",
        ],
        criticality="medium",
        output_validator=lambda snap: snap.get("profiles_processed", 0) >= 0,
        freshness_hours=170,
        estimated_claude_cost=0.0,
    ),
    ManifestEntry(
        name="shadow_mark_to_market",
        script="scripts/shadow_mark_to_market.py",
        pipeline_name="shadow_mark_to_market",
        schedule="0 18 * * 1-5",
        schedule_desc="6:00 PM PDT weekdays",
        expected_steps=[
            "shadow_mtm:load_positions",
            "shadow_mtm:fetch_prices",
            "shadow_mtm:process_positions",
        ],
        criticality="medium",
        dependencies=["shadow_opener_morning", "shadow_opener_midday"],
        output_validator=lambda snap: isinstance(snap.get("total"), int),
        freshness_hours=26,
        estimated_claude_cost=0.0,
    ),
]
